import json
ev=[]
for line in open(r'D:\Study\CompliSense\_audit_a.jsonl',encoding='utf-8'):
    line=line.strip()
    if not line: continue
    d=json.loads(line); ev.append(d)
    snip=str(d.get('snippet') or d.get('response') or '')[:100]
...
if done:
    print('ANSWER_HEAD:', (done[0].get('response') or '')[:500].replace('\n',' | '))
    print('SOURCES', len(done[0].get('sources') or []))
